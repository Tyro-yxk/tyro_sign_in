...
if 'MEITUAN_COOKIE' in os.environ:
    meituanCookie = re.split("@|&", os.environ.get("MEITUAN_COOKIE"))
...
def waim(ck):
    ...
    data = {"gundamId": 531693, "instanceId": "17211892932540.47486483758713405", "actualLongitude": 108938560,
            "actualLatitude": 31805038, "needTj": False,
    ...
    notify.send_success('美团领外卖券', f"获得{len(b['data']['allCoupons'])}张优惠券")
...
def tuangou(ck):
    ...
    headers = {"Cookie": cookie,
               ...
               "User-Agent": "Mozilla/5.0 (Linux; Android 14; 2201122C Build/UKQ1.230917.001; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/116.0.0.0 Mobile Safari/537.36 XWEB/1160027 MMWEBSDK/20231105 MMWEBID/2247 MicroMessenger/8.0.44.2502(0x28002C37) WeChat/arm64 Weixin Android Tablet NetType/WIFI Language/zh_CN ABI/arm64 miniProgram/wxde8ac0a21135c07d"}
    url = 'https://mediacps.meituan.com/gundam/gundamGrabV4?gdBs=&yodaReady=h5&csecplatform=4&csecversion=2.4.0'
    data = {"gundamId": 567871, "instanceId": "17236365795140.22172928381444668", "actualLongitude": 108938560,
            "actualLatitude": 31805038, "needTj": False,
            "couponConfigIdOrderCommaString": "1819834260,1953473011,1893798091,1384683696,1137971534,1991988913,1651917378,1519357553",
            "couponAllConfigIdOrderString": "1819834260,1953473011,1893798091", "ctype": "wxapp", "platform": 11,
            "app": -1,
    ...
    for sj in b['data']['allCoupons']:
        print(f"{sj['couponName']}-{sj['amountLimit']}-{sj['couponAmount']}元-{sj['amountLimit']}-{sj['etime']}")
    notify.send_success('美团领团购券', f"获得{len(b['data']['allCoupons'])}张优惠券")
